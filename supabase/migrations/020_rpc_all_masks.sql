-- Replaces 017_rpc_add_village_green.sql.
-- Adds greenspace + water masks, raises min_area 25→200 m²,
-- replaces flat 'medium' confidence with size-based scoring.
...
begin
  ...
  -- Road mask (buffered in BNG at query time)
  select coalesce(
    st_union(
      st_transform(
  ...
  -- Water mask (buffered in BNG, same pattern as roads)
  select coalesce(
    st_union(
      st_transform(
        st_buffer(st_transform(wl.geometry, 27700), wl.buffer_m),
        4326
      )
    ),
    st_geomfromtext('GEOMETRYCOLLECTION EMPTY', 4326)
  ...
  into v_water_mask
  from water_links wl
  where st_intersects(wl.geometry, v_envelope);
