/**
 * load-bona-vacantia.ts
 *
 * Loads bona vacantia (Crown ownerless property) polygons from a GeoJSON file
 * into the bona_vacantia table.
 *
 * Data source:
 *   The BVD unclaimed estates list was removed from GOV.UK in July 2025
 *   following a fraud investigation. When it is restored, download from:
 *     https://www.gov.uk/government/collections/bona-vacantia
 *   Convert to GeoJSON with ogr2ogr if needed, then pass via --input.
 *
 * Usage:
 *   npx tsx scripts/data/load-bona-vacantia.ts --input path/to/bona_vacantia.geojson [--data-year YYYY]
 */
...
async function main() {
  ...
  if (!inputPath) {
    ...
    console.error('')
    console.error('The BVD unclaimed estates list was removed from GOV.UK in July 2025.')
    console.error('When restored, download from:')
    console.error('  https://www.gov.uk/government/collections/bona-vacantia')
    console.error('Then convert to GeoJSON and pass via --input.')
    process.exit(1)
  }
  ...
  const flush = async () => {
    ...
    const { error } = await supabase.from('bona_vacantia').insert(batch)
  ...
  for await (const feature of streamFeatures(inputPath)) {
    ...
    batch.push({
      bvd_ref: feature.properties?.bvd_ref ?? feature.properties?.BVD_REF ?? feature.properties?.reference ?? null,
      title: feature.properties?.title ?? feature.properties?.TITLE ?? feature.properties?.description ?? null,
